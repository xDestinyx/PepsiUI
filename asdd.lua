if game.CoreGui:FindFirstChild("PeachyHub") then
    game.CoreGui:FindFirstChild("PeachyHub"):Destroy()
end
local library = {
	WorkspaceName = "PeachyHub",
	flags = {},
	signals = {},
	objects = {},
	elements = {},
				...
				if getsynasset then
					...
					elseif #image > 14 and string.sub(image, 1, 14) == "synasseturl://" then
						local x, e = pcall(function()
							...
							local fold = isfolder("./Peachy Edition")
							if not fold then
								makefolder("./Peachy Edition")
							end
							fold = isfolder("./Peachy Edition/Themes")
							if not fold then
								makefolder("./Peachy Edition/Themes")
							end
							fold = isfolder("./Peachy Edition/Themes/SynapseAssetsCache")
							if not fold then
								makefolder("./Peachy Edition Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Peachy Edition/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./Peachy Edition/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./Peachy Edition/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "Peachy Edition" then
		library.WorkspaceName = convertfilename(windowName, "Peachy Edition")
	end
	...
	FunctionLibrary.Name = "PeachyHub"
	...
	tabsHolder.Image = "rbxassetid://5553946656"
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./Peachy Edition") then
						makefolder("./Peachy Edition")
					end
					local common_string = "./Peachy Edition/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./Peachy Edition/" .. tostring(custom_workspace or library.WorkspaceName)
						end
			...
			function sectionFunctions:AddDropdown(options, ...)
				...
				dropdownSelection.ZIndex = 5
				dropdownSelection.Font = Enum.Font.Code
				dropdownSelection.Text = (passed_multiselect == "string" and multiselect) or ((multiselect and table.concat(selectedOption, ", "):len() < 30) and table.concat(selectedOption, ", ") or "Select Item(s)") or (multiselect and (blankstring or "Select Item(s)")) or (selectedOption and tostring(selectedOption)) or blankstring or "No Blank String"
				dropdownSelection.TextColor3 = library.colors.otherElementText
				colored[1 + #colored] = {dropdownSelection, "TextColor3", "otherElementText"}
				dropdownSelection.TextSize = 14
				dropdownSelection.TextXAlignment = Enum.TextXAlignment.Left
				...
				local Set = (multiselect and function(t, dat)
					...
					dropdownSelection.Text = ((multiselect and table.concat(selectedOption, ", "):len() < 30) and table.concat(selectedOption, ", ") or "Select Item(s)") or (passed_multiselect == "string" and multiselect) or blankstring or "Select Item(s)"
					if proceed and callback then
						task.spawn(callback, selectedOption, cloned)
					end
					return selectedOption
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		for _, v in next, {{"Main", "main"}, {"Background", "background"}, {"Outer Border", "outerBorder"}, {"Inner Border", "innerBorder"}, {"Top Gradient", "topGradient"}, {"Bottom Gradient", "bottomGradient"}, {"Section Background", "sectionBackground"}, {"Section", "section"}, {"Element Text", "elementText"}, {"Other Element Text", "otherElementText"}, {"Tab Text", "tabText"}, {"Element Border", "elementBorder"}, {"Selected Option", "selectedOption"}, {"Unselected Option", "unselectedOption"}, {"Hovered Option Top", "hoveredOptionTop"}, {"Unhovered Option Top", "unhoveredOptionTop"}, {"Hovered Option Bottom", "hoveredOptionBottom"}, {"Unhovered Option Bottom", "unhoveredOptionBottom"}} do
			...
			if RainbowText and (nam == "Tab Text" or nam == "Main" or nam == "Element Text" or nam == "Other Element Text") then
